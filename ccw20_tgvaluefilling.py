...
tgarr=gridasciitonumpyarrayint(myworkspace+"/chli_tg.asc")[0]
...
#*****************************************************************************************************************
#loop through tgarr until no cell with value == -1 exists
count9999s = 0
i=1
while i<rasrows-1:
    #print i
    ...
    j=1
    while j<rascols-1:
        if tgarr[i,j]==-1 and 1<=fdarr[i,j] <=128 and demarr[i,j]!=NODATA_value:
...
numpy.savetxt(myworkspace+"/"+"chli_tgfilled"+".asc", tgarr, fmt='%i',delimiter=' ',newline='\n', header=headerstr, comments='')
